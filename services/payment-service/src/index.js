...
app.get('/api/payments/vnpay-return', async (req, res) => {
  try {
    ...
    if (secureHash === signed && responseCode === '00') {
      ...
      await pool.query(
        `UPDATE payments 
         SET payment_status = 'completed', payment_method = 'vnpay', paid_at = NOW()
         WHERE booking_id = $1`,
        [bookingId]
      );
...
app.get('/api/payments/mock-success/:bookingId', async (req, res) => {
  try {
    ...
    await pool.query(
      `UPDATE payments 
       SET payment_status = 'completed', payment_method = 'mock', 
...
app.post('/api/payments/: id/cash', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE payments 
       SET payment_method = 'cash', payment_status = 'completed', 
           transaction_id = $1, paid_at = NOW()
       WHERE id = $2
       RETURNING *`,
      [transactionId, id]
